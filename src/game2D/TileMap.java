...
// extends Jpanel so that we can interact with but
public class TileMap
{
	...
	{
		if (!valid(x,y) || tmap[x][y] == null) return '?';
	...
	/**
	 * Checks if the tile at position 'x,y' is a blank tile or not.
	 * @param x The x tile coordinate (in tiles, not pixels)
	 * @param y The y tile coordinate (in tiles, not pixels)
	 * @return whether the tile at position 'x,y' is a blank tile or not.
	 */
	public boolean hasTile(int x, int y)
	{
		if (getTile(x,y).getCharacter() != '.') return true;
		return false;
	}
