...
public class Sprite {
    ...
    public void update(long elapsedTime) {
        if (!render) return;
        // update position (whatever pos was before + change of pixels in millis * time passed)
        x += dx * elapsedTime;
        y += dy * elapsedTime;
...
//        // divide the image into 4 quadrants and call drawArc on each
//        g.drawArc((int) x + xoff, (int) y + yoff, img.getWidth(null) / 2, img.getHeight(null) / 2, 0, 360);
//        g.drawArc((int) x + xoff + img.getWidth(null) / 2, (int) y + yoff, img.getWidth(null) / 2, img.getHeight(null) / 2, 0, 360);
//        g.drawArc((int) x + xoff, (int) y + yoff + img.getHeight(null) / 2, img.getWidth(null) / 2, img.getHeight(null) / 2, 0, 360);
//        g.drawArc((int) x + xoff + img.getWidth(null) / 2, (int) y + yoff + img.getHeight(null) / 2, img.getWidth(null) / 2, img.getHeight(null) / 2, 0, 360);
    ...
    // method to get bounding box of sprite
    public Rectangle getBoundingBox(Sprite s) {
        return new Rectangle((int) s.getX(), (int) s.getY(), (int) s.getWidth(), (int) s.getHeight());
    }
    ...
    /**
     * Set the animation to the one specified
     *
     * @param animationName specifies which animation to use
     * @param animationMap  map collecting all the animations
     */
    public void setAnimationTo(String animationName, Map animationMap) {
        this.anim = (Animation) animationMap.get(animationName);
    }

    // check if the sprite is falling or flying. Set the animation accordingly
    public void checkState(Map animationMap) {
        if (this.dy > 0) {
            this.setAnimationTo("falling", animationMap);
        } else if (this.dy < 0) {
            this.setAnimationTo("flying", animationMap);
        }
    }
